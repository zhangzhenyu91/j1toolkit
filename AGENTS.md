...
- `design/index.html` —— UI 设计稿（方案五为定稿，见第四节）
...
**实现方式**：组件使用 tdesign-miniprogram，在全局样式（`app.wxss`）覆盖 `--td-*` CSS 变量注入下列 token；警示斜纹、描边徽章等特征元素用 wxss 自绘。所有页面及后续应用子页面必须沿用同一套 token，与本体风格一致。参照实现：`design/style-5.html`。
...
- `design/` —— UI 设计稿（`style-5.html` 为定稿参照，其余为历史方案，保留不删）
- 根目录 —— 项目文档与规则文件
